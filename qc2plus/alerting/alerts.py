...
class AlertManager:
    ...
    def __init__(self, alerting_config: Dict[str, Any]):
        ...
        # Alert thresholds
        self.thresholds = alerting_config.get(
            "thresholds",
            {
    ...
    def _analyze_results_for_alerting(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for model_name, model_results in results.get("models", {}).items():
            ...
            for analyzer_name, analyzer_result in model_results.get(
                "level2", {}
            ).items():
                if not analyzer_result.get("passed", True):
                    ...
                    if anomalies_count > 0:
                        ...
                        failure_info = {
                            ...
                            "explanation": self._get_level2_explanation(
                                analyzer_name, analyzer_result
                            ),
        ...
        total_failures = (
            len(critical_failures) + len(high_failures) + len(medium_failures)
        )
        failure_rate = total_failures / max(results.get("total_tests", 1), 1)

        should_alert = (
            len(critical_failures) >= self.thresholds["critical_failure_threshold"]
            or failure_rate >= self.thresholds["failure_rate_threshold"]
        )
    ...
    def _send_summary_alert(
        self, alert_info: Dict[str, Any], results: Dict[str, Any]
    ) -> None:
    ...
    def _send_email_alert(
        self, alert_data: Dict[str, Any], individual: bool = False
    ) -> None:
        """Send email alert"""
    ...
    def _create_summary_email_html(self, alert_data: Dict[str, Any]) -> str:
        ...
        success_rate = (
            alert_data["passed_tests"] / max(alert_data["total_tests"], 1)
        ) * 100
    ...
    def _create_slack_individual_payload(
        self, alert_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Create Slack payload for individual alert"""
        test_type_display = (
            "Level 1" if alert_data.get("test_type") == "level1" else "Level 2"
        )

        fields = [
            {
                "title": "Model",
                "value": alert_data["model"],
                "short": True,
            },
            {
                "title": "Test",
                "value": alert_data["test"],
                "short": True,
            },
            {
                "title": "Environment",
            ...
            {
                "title": "Test Type",
                "value": test_type_display,
                "short": True,
            },
            {
                "title": "Failed/Total",
                "value": f"{alert_data.get('failed_rows', 0)}/{alert_data.get('total_rows', 0)}",
                "short": True,
            },
            {
                "title": "Run ID",
        ...
        if alert_data.get("query") and alert_data.get("test_type") == "level1":
            query_text = alert_data["query"][:500] + (
                "..." if len(alert_data["query"]) > 500 else ""
            )
            fields.append(
                {
                    "title": "Query",
    ...
    def _create_slack_summary_payload(
        self, alert_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        fields = [
            {
                "title": "📊 Total Tests",
                "value": str(alert_data["total_tests"]),
                "short": True,
            },
            {
                "title": "✅ Success Rate",
            ...
            {
                "title": "🚨 Critical",
                "value": str(alert_data["critical_failures"]),
                "short": True,
            },
            {
                "title": "⚠️ High",
                "value": str(alert_data["high_failures"]),
                "short": True,
            },
            {
                "title": "📋 Medium",
                "value": str(alert_data["medium_failures"]),
                "short": True,
            },
            {
                "title": "⏱️ Duration",
                "value": f"{alert_data['execution_duration']}",
                "short": True,
            },
            {
                "title": "🔍 Run ID",
    ...
    def _create_teams_individual_payload(
        self, alert_data: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _create_teams_summary_payload(
        self, alert_data: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            "sections": [
                {
                    ...
                    "facts": [
                        ...
                        {
                            "name": "Passed",
                            "value": str(alert_data["passed_tests"]),
                        },
                        {
                            "name": "Failed",
                            "value": str(alert_data["failed_tests"]),
                        },
                        {
                        ...
                        {
                            "name": "Medium Failures",
                            "value": str(alert_data["medium_failures"]),
                        },
                        {
                            "name": "Execution Duration",
                            "value": f"{alert_data['execution_duration']}s",
                        },
                        {
                            "name": "Run ID",
                            "value": alert_data["run_id"],
    ...
    def _determine_level2_severity(
        self, analyzer_result: Dict[str, Any], anomalies_count: int
    ) -> str:
        ...
        temporal_anomalies = details.get("temporal_correlation", {}).get(
            "anomalies", []
        )
        for anomaly in temporal_anomalies:
            if (
                anomaly.get("severity") == "high"
                or anomaly.get("anomaly_type") == "sudden_change"
            ):
    ...
    def _get_level2_explanation(
        self, analyzer_name: str, analyzer_result: Dict[str, Any]
    ) -> str:
        ...
        base_explanation = explanations.get(
            analyzer_name,
            f"Level 2 anomaly analysis of type '{analyzer_name}' detected unusual patterns in the data.",
        )
    ...
    def _get_correlation_explanation(self, details: Dict[str, Any]) -> str:
        ...
        temporal_count = len(
            details.get("temporal_correlation", {}).get("anomalies", [])
        )
    ...
    def _extract_level2_examples(self, analyzer_result: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if "temporal_correlation" in details:
                temporal_anomalies = details["temporal_correlation"].get(
                    "anomalies", []
                )
            ...
            # For temporal analysis (general)
                ...
                for metric, results in list(individual.items())[
                    :3
                ]:  # Limit to 3 metrics
                    ...
                    if anomalies:
                        ...
                        examples.append(
                            f"{metric}: {anomaly_type} (z-score: {z_score:.2f})"
                        )
                        ...
                        for i, anomaly in enumerate(generic_anomalies):
                            if isinstance(anomaly, dict):
                                score = anomaly.get(
                                    ...
                                    anomaly.get(
                                        "magnitude",
                                        anomaly.get("correlation", i + 1),
                                    ),
                                )
                                var_pair = anomaly.get(
                                    "variable_pair",
                                    anomaly.get("variable", f"Item {i + 1}"),
                                )
    ...
    def _collect_failure_details(
        ...
        all_failures = alert_info["high_failures"] + alert_info["medium_failures"]
