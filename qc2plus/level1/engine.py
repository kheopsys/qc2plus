...
# from qc2plus.level1.utils import build_sample_clause, get_macro_help
...
class Level1Engine:
    ...
    def __init__(self, connection_manager: Optional[ConnectionManager] = None):
        ...
        for macro_name, macro_template in SQL_MACROS.items():
            self.jinja_env.globals[macro_name] = self._create_macro_function(
                macro_template
            )
    ...
    def run_tests(
        self,
        model_name: str,
        level1_tests: List[Dict[str, Any]],
    ...
    def _run_single_test(
        ...
        base_result = {
            ...
            "explanation": self._get_test_explanation(
                test_type, test_params
            ),  # New field for human-readable explanation
        ...
        try:
            ...
            if len(df) == 0:
                ...
                examples = self._extract_examples_from_results(
                    df, test_type, test_params
                )
    ...
    def compile_test(
        ...
        db_type = (
            self.connection_manager.db_type if self.connection_manager else "postgresql"
        )
        ...
        context = {
            ...
            "schema": (
                self.connection_manager.config.get("schema", "public")
    ...
    def validate_test_config(
        self, test_type: str, test_params: Dict[str, Any]
    ) -> List[str]:
        ...
        if test_type in [
            ...
            if "threshold_type" not in test_params:
                issues.append(
                    "Test statistical_threshold requires 'threshold_type' parameter"
                )
            elif test_params["threshold_type"] not in [
                "absolute",
                "relative",
            ]:
            ...
            if "threshold_value" not in test_params:
                issues.append(
                    "Test statistical_threshold requires 'threshold_value' parameter"
                )
            ...
            if "accepted_benchmark_values" in test_params:
                ...
                if not isinstance(benchmark_values, dict):
                    ...
                    if abs(total_pct - 100) > 5:  # Allow 5% tolerance
                        issues.append(
                            f"benchmark_values percentages should sum to ~100% (got {total_pct}%)"
                        )
    ...
    def _get_test_explanation(self, test_type: str, test_params: Dict[str, Any]) -> str:
        ...
        return explanations.get(
            test_type,
            f"Test de qualité de type '{test_type}' sur le modèle de données.",
        )

    def _extract_examples_from_results(
        self,
        df: pd.DataFrame,
        test_type: str,
        test_params: Dict[str, Any],
        ...
        try:
            ...
            if test_type in [
                ...
                if (
                    "value" in df.columns
                    and "actual_pct" in df.columns
                    and "expected_pct" in df.columns
                ):
            ...
            logging.warning(
                f"Could not extract examples for test {test_type}: {str(e)}"
            )
            if len(df) > 0 and len(df.columns) > 0:
                first_col = df.columns[0]
                examples = [
                    str(val) for val in df[first_col].head(max_examples).tolist()
                ]
