...
class CorrelationAnalyzer:
    ...
    def analyze(self, model_name: str, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            window_days = config.get(
                "window_days", None
            )  # for temporal correlation analysis
            sample_size = config.get(
                "sample_size", 10000
            )  # used for calculating correlation with no time windows
            warnings.filterwarnings(
                "ignore", category=RuntimeWarning
            )  # Suppress warnings from correlation calculations
            # Validate configuration
            if len(variables) < 2:
                raise ValueError(
                    "At least 2 variables required for correlation analysis"
                )
            ...
            # Detect temporal correlation changes (seulement si date_column
            # existe)
            temporal_results = {
                "passed": True,
                "anomalies_count": 0,
            ...
            total_anomalies = results.get("anomalies_count", 0) + temporal_results.get(
                "anomalies_count", 0
            )
    ...
    def _perform_correlation_analysis(
        ...
        data: pd.DataFrame,
        variables: List[str],
        expected_correlation: Optional[float],
        threshold: float,
            ...
            for j in range(i + 1, len(variables)):
                ...
                try:
                    if correlation_type == "pearson":
                        ...
                        corr_coef = np.corrcoef(clean_data[var1], clean_data[var2])[
                            0, 1
                        ]
                        p_value = None
                ...
                if (
                    expected_correlation
                    and abs(expected_correlation) > 0.5
    ...
    def _detect_temporal_correlation_changes(
        self,
        model_name: str,
        variables: List[str],
        date_column: str,
        ...
        try:
            # Get historical correlation data (weekly windows over last 3
            # months)
            schema = self.connection_manager.config.get("schema", "public")
            db_type = self.connection_manager.db_type
            funcs = DB_LEVEL2_FUNCTIONS.get(db_type, DB_LEVEL2_FUNCTIONS["postgresql"])
            ...
            for i in range(len(variables)):
                for j in range(i + 1, len(variables)):
                    ...
                    if (
                        var1 not in historical_data.columns
                        or var2 not in historical_data.columns
                    ):
                        continue
                        ...
                        if len(clean_window) >= 2:
                            try:
                                ...
                                if not np.isnan(corr):
                                    ...
                                    dates.append(
                                        historical_data.iloc[idx + 2]["week_start"]
                                    )
                    ...
                    if len(correlations) >= 3:
                        ...
                        if len(correlations) >= 4:
                            ...
                            if (abs(early_corr) > 0.5 or abs(late_corr) > 0.5) and abs(
                                degradation
                            ) > 0.3:
                                results["passed"] = False
                                results["anomalies_count"] += 1
