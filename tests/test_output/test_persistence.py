...
class TestLevel1Engine:
    ...
    def test_available_tests_list(self):
        ...
        # ✅ FIX: Utiliser les vrais noms de tests du code
        expected_tests = [
            'unique', 'not_null', 'email_format', 
            'relationship',  # ← Changé de 'foreign_key' à 'relationship'
            'future_date', 'statistical_threshold'
        ]
        
        for test in expected_tests:
            assert test in available_tests, f"Test '{test}' not found in {available_tests}"
